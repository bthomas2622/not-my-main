            ...
            for (const game of data.data.Catalog.searchStore.elements) {
                ...
                if (game.promotions) {
                    if (game.promotions.promotionalOffers.length !== 0) {
                        hasPromoOffers = true;
                        inThisWeek = dayjs() > dayjs(game.promotions.promotionalOffers[0].promotionalOffers[0].startDate) && dayjs() < dayjs(game.promotions.promotionalOffers[0].promotionalOffers[0].endDate);        
                    }
                    if (game.promotions.upcomingPromotionalOffers.length !== 0) {
                        hasUpcomingPromoOffers = true;
                        inNextWeek = dayjs().add(1, "week") > dayjs(game.promotions.upcomingPromotionalOffers[0].promotionalOffers[0].startDate) && dayjs().add(1, "week") < dayjs(game.promotions.upcomingPromotionalOffers[0].promotionalOffers[0].endDate);
